...
function App() {
  ...
  useEffect(() => {
    ...
    fetchBatches();
  }, [token, isPublicViewer, fetchBatches]);
  ...
  }, [batches, setActiveBatch]);

  useEffect(() => {
    if (!token || !activeBatchId || isPublicViewer) {
      return;
    }
    fetchLogs();
    if (canViewFinancial) {
  ...
  }, [token, activeBatchId, isPublicViewer, canViewFinancial, fetchLogs, fetchTransactions]);
      ...
      <div className="bg-app-bg text-app-text min-h-screen flex min-w-0 flex-col md:flex-row transition-colors duration-300 font-sans">
        ...
        <div className="flex-1 min-w-0 min-h-screen flex flex-col overflow-x-hidden">
          ...
          <div className="flex-1 min-w-0">
            {currentScreen === 'today' && (
              <TodayOperations setActiveScreen={setActiveScreen} />
            )}
            ...
            {currentScreen === 'dashboard' && (
              <Dashboard setActiveScreen={setActiveScreen} />
            )}
