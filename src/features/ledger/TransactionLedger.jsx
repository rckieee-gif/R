    ...
    const fetchMasterData = async () => {
      ...
      try {
        const [buildingData, categoryData, nextStakeholders, feedData] = await Promise.all([
          ...
          apiClient.get('/api/categories', { expectArray: true }).catch((err) => {
            console.warn('Falling back to default categories:', err);
            return DEFAULT_CATEGORY_DATA;
          }),
          fetchStakeholders().catch((err) => {
            console.warn('Falling back to default stakeholders:', err);
            return DEFAULT_STAKEHOLDER_DATA;
          }),
